  ...
  return <div className="page training-page">
    ...
    <div className="training-layout">
      <section className="panel training-config">
        <TrainingSection title={text.identity}><div className="training-grid"><label>{text.aiIsBe}<select value={profileId} onChange={(event) => setProfileId(event.target.value)}>{profiles.map((item) => <option key={item.id} value={item.id}>{aiIsBeDisplayName(item)}</option>)}</select></label></div><small>{provider ? `${provider.label} · ${viewerModel?.displayName ?? text.noModel}` : text.noProvider}</small></TrainingSection>
