  ...
  return <FormDialog title={copy.createProfile} onCancel={onCancel} modalClassName="profile-edit-modal"><form className="profile-edit-form" onSubmit={(event) => void submit(event)}>
    ...
    <fieldset className="profile-edit-ai profile-create-ai"><legend>{copy.profileAiDefaults}</legend>
      ...
      {advancedOpen && <section id="profile-create-advanced" className="profile-create-advanced" aria-label={copy.advancedOptions}>
        ...
        <label>{copy.modelSearch}<input value={setup.modelSearch} onChange={(event) => setup.setModelSearch(event.target.value)} placeholder={copy.modelSearchPlaceholder} disabled={!setup.selectedProvider} /></label>
        <label>{copy.defaultViewerModel}<select value={setup.viewerModelId} onChange={(event) => setup.selectViewerModel(event.target.value)} disabled={!setup.selectedProvider}><option value="">{setup.visibleViewerModels.length ? copy.selectModel : copy.noMatchingModels}</option>{setup.visibleViewerModels.map((model) => <option key={model.modelId} value={model.modelId}>{model.favorite ? "★ " : model.recommended ? "✦ " : ""}{model.displayName}</option>)}</select></label>
