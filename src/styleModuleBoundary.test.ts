...
const files = ["base.css", "shared.css", "conversations.css", "sessions.css", "settings.css", "monitor.css", "training-research.css", "ai-center.css"] as const;
...
describe("Etap 7 style boundaries", () => {
  it("keeps app.css as the ordered style entry point", () => {
    ...
    const expected = files.map((file) => `@import "./${file}";`).join("\n") + "\n";
  ...
  it("keeps each style area in an explicit module", () => {
    ...
    for (const file of files) {
      ...
      expect(fs.readFileSync(path.join(root, file), "utf8").trim().length).toBeGreaterThan(0);
    }
  });
});
