  ...
  return <div className="page training-page">
    ...
    <div className="training-layout">
      <section className="panel training-config">
        ...
        {mode === "partial" && <TrainingSection title={text.categories}><div className="training-category-grid">{TRAINING_CATEGORIES.map((category) => { const available = targets.filter((target) => target.collection === "training" && target.sourceMetadata.category === category).length; return <label key={category}><span>{TRAINING_CATEGORY_LABELS[category][settings.interfaceLanguage]}<small>{text.factory} · {text.available}: {available}</small></span><input type="number" min={0} max={available} value={counts[category] ?? 0} onChange={(event) => setCounts((current) => ({ ...current, [category]: Math.max(0, Number(event.target.value) || 0) }))} /></label>; })}<label className="training-my-targets"><span>{text.user}<small>{text.available}: {targets.filter((target) => target.collection === "user").length}</small></span><input type="number" min={0} max={targets.filter((target) => target.collection === "user").length} value={myTargetsCount} onChange={(event) => setMyTargetsCount(Math.max(0, Number(event.target.value) || 0))} /></label></div></TrainingSection>}
