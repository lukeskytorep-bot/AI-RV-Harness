...
function ResearchProjectView({ copy, repository, project, onRefresh, onBack }: { copy: Copy; repository: AppRepository; project: ResearchProjectRecord; onRefresh: () => Promise<void>; onBack: () => void }) {
  ...
  return <div className="research-project-view">
    ...
    {completedAssignments.length > 0 && <section className="panel research-session-history"><div className="research-section-head"><strong>{copy.completedSessions}</strong><small>{completedAssignments.length}</small></div><div>{completedAssignments.map((assignment, index) => <button key={assignment.id} className={selectedSessionId === assignment.sessionId ? "active" : ""} onClick={() => setSelectedSessionId(assignment.sessionId!)}>{index + 1}. {assignment.anonymousSessionId} · {assignment.status}</button>)}</div></section>}
