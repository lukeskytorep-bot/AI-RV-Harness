...
| `src/storage/contracts/sessionsRepository.ts` | `src/storage/browser/sessionsRepository.ts` | `src/storage/sqlite/sessionsRepository.ts` | RV Session records, ordered events, sealed evidence, immutable snapshots, Reveal, post-Reveal transcript and target clarifications delegate here. Research frozen-score knowledge stays explicit at the facade boundary; Monitor/Judge/Research persistence remains outside this split. |
