...
| Database migrations and native transactions | `src-tauri/src/migrations.rs`, `src-tauri/src/database.rs` and `src-tauri/src/storage.rs` | `migrations.rs` is the single ordered 001–024 registry and current-version source. Only an exact compatible green v23 database upgrades to v24; older public/legacy schemas are stopped by the compatibility epoch gate before plugin migration. Transactions and database validation remain with their focused native owners. |
