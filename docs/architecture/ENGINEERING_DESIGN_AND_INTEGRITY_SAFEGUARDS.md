...
UX-DATA-9 and the later Viewer Learning extension turn the accepted UX/data invariants into permanent build checks. `npm run verify:ux-data` verifies the contiguous migration registry through 024, flat Conversation product boundary, shared dialog/model-route boundaries, unified lifecycle surface, Viewer Notes/target preservation markers and Field Guide schema markers. A native Rust test upgrades only the exact accepted green v23 boundary to v24, verifies that existing data and provenance survive, preserves a custom Profile Viewer prompt as an unresolved legacy baseline without inventing identity/language, and requires a clean `PRAGMA foreign_key_check`. Older public/legacy databases are handled by the compatibility epoch gate rather than silently entering this controlled upgrade path.
