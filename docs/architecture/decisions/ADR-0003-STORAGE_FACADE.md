...
**Updated:** 9 September 2026 — UX-DATA-5 removes `ChatThreadGroup` from the product contract while retaining schema compatibility
...
The fourth extraction is Workspaces and Conversations. At the time of Etap 5 it preserved Workspace lifecycle, Thread-group lifecycle, Conversation/Manual RV records and messages together behind `WorkspacesConversationsRepository` so structural modularization did not change product behavior. UX-DATA-5 later executes the separately planned product migration: `ChatThreadGroup` methods are removed from `AppRepository` and the internal contract, new Conversation/Manual RV rows use `thread_group_id = NULL`, and archive/restore operates directly on the conversation record with Workspace as the only product parent. The physical table/column remain for backward-compatible reads; the obsolete global `ChatThreadGroup` product type is removed in UX-DATA-9 and legacy fixture typing is kept local to compatibility tests. No schema migration is required. Workspace Sources remain outside this contract.
...
Etap 5 itself changed no schema, migration, table, local-storage key or serialized record format. UX-DATA-5 also intentionally leaves the physical `chat_thread_groups` table and nullable `thread_group_id` column in place, using them only as dormant compatibility metadata for old data.
